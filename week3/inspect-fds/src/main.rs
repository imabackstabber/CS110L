...
fn main() {
    ...
    let res = ps_utils::get_target(target);
    let possible_proc = match res{
        Ok(process) => process,
        Err(error) => panic!("Problem when encountering: {}",error)
    };
    let proc = match possible_proc{
        Some(proc) => proc,
        None => {
    ...
    println!("{}",proc);
    let pchilds = ps_utils::get_child_processes(proc.pid)
                        .unwrap();
