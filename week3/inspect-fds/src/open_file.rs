use nix::libc::_PC_REC_MAX_XFER_SIZE;
use regex::Regex;
...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        // unimplemented!();
        let pbuf = fs::read_link(
            std::path::Path::new(&format!("/proc/{}/fd/{}",pid,fd))).ok()?;
        let _pname = OpenFile::path_to_name(pbuf.to_str()?);
        
        // /proc/<PID>/fdinfo is not implemented on wsl
        // let pstat = fs::read_to_string(
        //     std::path::Path::new(&format!("/proc/{}/fdinfo/{}", pid,fd))).ok()?;
        // let pcursor = OpenFile::parse_cursor(&pstat)?;
        // let paccess = OpenFile::parse_access_mode (&pstat)?;
        // Some(OpenFile{
        //     name: pname,
        //     cursor: pcursor,
        //     access_mode: paccess,
        ...
        // we will return a fake info here
        Some(OpenFile { name: String::from("<terminal>"), cursor: 0, access_mode: AccessMode::ReadWrite })
    }
