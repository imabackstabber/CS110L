...
use std::env;
use std::fs::File; use std::i64::MAX;
// For read_file_lines()
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let num_cols = seq2.len();
    let mut c = Grid::new(num_rows+1, num_cols+1);
    c.clear();
    for i in 1..=num_rows{
        for j in 1..=num_cols{
            if seq1[i-1] == seq2[j-1]{
                // c[i+1][j+1] = c[i][j]+1;
                c.set(i, j, c.get(i-1, j-1).unwrap() + 1);
            } else{
...
fn main() {
    ...
    // unimplemented!();
    let ans1 = read_file_lines(filename1).expect(&format!("fail to read {}", filename1));
    let ans2 = read_file_lines(filename2).expect(&format!("fail to read {}", filename2));
    let c = lcs(&ans1,&ans2);
    print_diff(&c, &ans1, &ans2, ans1.len(), ans2.len());
